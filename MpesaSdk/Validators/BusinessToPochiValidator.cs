...
{
	public class BusinessToPochiValidator : AbstractValidator<BusinessToPochiRequest>
	{
		public BusinessToPochiValidator()
		{
			...
			RuleFor(x => x.PartyB)
				...
				.SetValidator(new PhoneNumberValidator<BusinessToPochiRequest, string>())
			...
			RuleFor(x => x.QueueTimeOutUrl)
				.NotNull().WithMessage("QueueTimeOutURL is required")
				.Must(x => LinkMustBeAUri(x))
				.WithMessage("{PropertyName} - The queue timeout url should be a valid secure url.");
			
			RuleFor(x => x.ResultUrl).NotNull()
